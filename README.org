...
*I will only introduce length-field-based-frame-codec here for the reason it`s the monst complicated codec。
*** length-field-based-frame-codec
length-field-based-frame is one frame package way who makes a fixed length field as the header of a frame and represents the length of the whole msg or only content。
#+begin_src text
...
For how config struct`s parameters controlled the format of the frame. I has copy descriptions from Netty javadoc, I have implemented all of them in tests/read-write-frame-tests.lisp
